...
class DetailView extends WatchUi.View {
    ...
    // Small red "L" shown at the top only while this aircraft is the locked
    // target (unlocked needs no marker - the footer line states the state).
    // Placed in the always-visible zone: top-centre on round (corners are
    // clipped), top-right on square, above the name so it doesn't cover it.
    private function drawLockBadge(dc as Dc, w as Number, h as Number,
                                   round as Boolean) as Void {
        if (_model.targetPlane != _plane) { return; }
        var txt = "L";
        var font = Graphics.FONT_SMALL;
        var tw = dc.getTextWidthInPixels(txt, font);
        var fh = dc.getFontHeight(font);
        var cx = round ? (w / 2) : (w - tw / 2 - 12);
        var cy = round ? ((h * 0.06).toNumber() + fh / 2) : (4 + fh / 2);
